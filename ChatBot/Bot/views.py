...
def whoIs(query, sessionID="general"):
    ...
    try:
        response = requests.get(
            'http://api.stackexchange.com/2.2/tags/' + query + '/wikis?site=stackoverflow')
        data = response.json()
...
def results(query, sessionID="general"):
    ...
    query_list = [x for x in query_list if x not in [
        'posted', 'questions', 'recently', 'recent', 'display', '', 'in', 'of', 'show']]
    # print(query_list)
    if len(query_list) == 1:
        # print('con 1')
        try:
            ...
            data = response.json()
            data_list = [str(i+1)+'. ' + data['items'][i]['title']
                         for i in range(5)]
        ...
        # print('con 3')
        query_list = [x for x in query_list if x not in [
            'which', 'where', 'whos', 'who\'s' 'is', 'are', 'answered', 'not', 'unanswered', 'for']]
        # print(query_list)
        if len(query_list) == 1:
            try:
                response = requests.get(
                ...
                data = response.json()
                data_list = [str(i+1)+'. ' + data['items'][i]
...
def Home(request):
    return render(request, "Bot/home.html", {'home': 'active', 'chat': 'chat'})
...
def Post(request):
    ...
    if request.method == "POST":
        ...
        chat.conversation["general"].append(
            '<br/>'.join(['ME: '+query, 'BOT: '+response]))
